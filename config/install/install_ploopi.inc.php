...
function ploopi_create_site(&$arrInstallInfos)
{
  ...
  $strHash = hash('sha256', "{$_SESSION['install']['<SECRETKEY>']}/{$_SESSION['install']['<ADMIN_LOGIN>']}/".hash('sha256', $strAdminPassword));

  $intRequest = $objPDO->exec("UPDATE `ploopi_user` SET `login` = '{$_SESSION['install']['<ADMIN_LOGIN>']}', `password` = '{$strHash}', email = '{$_SESSION['install']['<ADMIN_MAIL>']}' WHERE  `login` = 'admin'");

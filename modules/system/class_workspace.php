	...
	**/
	...
	function getsharedmodules($herited = false)
	{
	 ...
	 function getprofiles()
	 {
		...
		// select profiles
		$select = 	"
					SELECT 		ploopi_profile.*,
								ploopi_workspace.label as labelworkspace
					FROM 		ploopi_profile,
								ploopi_workspace
					WHERE 		ploopi_profile.id_workspace = ploopi_workspace.id
					AND			ploopi_profile.id_workspace = {$this->fields['id']}
					ORDER BY 	ploopi_workspace.label
					";

		$result = $db->query($select);

		while ($profile = $db->fetchrow($result,MYSQL_ASSOC))
		{

...
switch ($_SESSION['system']['level'])
{
					...
					switch($op)
					{
						...
						case 'delete' :
							...
							if (!$sizeof_groups && !$sizeof_users)
							{
								...
								ploopi_redirect("{$scriptenv}?groupid={$idfather}");
							}
	...
	case _SYSTEM_WORKSPACES :
		...
		if ($_SESSION['ploopi']['adminlevel'] >= _PLOOPI_ID_LEVEL_GROUPADMIN)
		{
				...
				if ($_SESSION['ploopi']['modules'][_PLOOPI_MODULE_SYSTEM]['system_use_profiles'])
				{
					$toolbar['tabProfiles'] = array(
														'title' 	=> _SYSTEM_LABELICON_PROFILES,
														'url'		=> "{$scriptenv}?wspToolbarItem=tabProfiles",
														'icon'	=> "{$_SESSION['ploopi']['template_path']}/img/system/icons/tab_profile.png"
													);
				}
		...
		<tr>
			...
			switch($_SESSION['system']['wspToolbarItem'])
			{
				...
				// ----------------
				case 'tabProfiles':
					include_once './modules/system/admin_index_profiles.php';
				break;

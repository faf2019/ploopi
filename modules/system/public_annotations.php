				...
				for  ($i=0; $i<$nb_anno_page && !empty($annotation); $i++)
				{
					...
					<div class="system_annotations_row" style="background-color:<? echo $color; ?>">
						<div class="system_annotations_title">
							<? echo htmlentities($annotation['title']); ?>
						</div>
						...
						<div class="system_annotations_module">
							<a href="<? echo ploopi_urlencode("{$scriptenv}?ploopi_mainmenu=1&{$object_script}"); ?>"><b><? echo $annotation['module_name']; ?></b>  / <? echo htmlentities($annotation['object_name']); ?> / <? echo htmlentities($annotation['object_label']); ?></a>
						</div>
					</div>
					<?
					next($annotations);

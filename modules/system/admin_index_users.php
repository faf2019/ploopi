	...
	case 'tabUserAdd':
		...
		switch($op)
		{
			...
			case "save_user":
				...
				if (!empty($_POST['user_id']))
				{
					$user->open($_POST['user_id']);
					$testuser = "id = {$_POST['user_id']} AND";
				}
				else
				{
					$newuser = true;
				...
				if($_SESSION['ploopi']['modules'][$_SESSION['ploopi']['moduleid']]['system_same_login'])
				{
					$db->query("SELECT id FROM ploopi_user WHERE $testuser login = '$user_login' && password = '". md5($userx_password) ."'");
					if($db->numrows()) ploopi_redirect("$scriptenv?op=manage_account&error=passrejected");
				}
				else
				{
					$db->query("SELECT id FROM ploopi_user WHERE $testuser login = '$user_login'");
					if($db->numrows()) ploopi_redirect("$scriptenv?op=manage_account&error=login");
				}
				...
				{
					$user->fields['password'] = md5($userx_password);
					if ($_SESSION['ploopi']['modules'][_PLOOPI_MODULE_SYSTEM]['system_generate_htpasswd']) system_generate_htpasswd($user->fields['login'], $userx_password);
				}
				elseif ($userx_password != $userx_passwordconfirm) $passwordok = false;

				if ($newuser) ploopi_create_user_action_log(_SYSTEM_ACTION_CREATEUSER, "{$user->fields['login']} - {$user->fields['lastname']} {$user->fields['firstname']} (id:{$user->fields['id']})");
				else ploopi_create_user_action_log(_SYSTEM_ACTION_MODIFYUSER, "{$user->fields['login']} - {$user->fields['lastname']} {$user->fields['firstname']} (id:{$user->fields['id']})");
				$user->save();

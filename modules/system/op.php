...
if ($_SESSION['ploopi']['connected'] && $_SESSION['ploopi']['moduleid'] == _PLOOPI_MODULE_SYSTEM)
{
	switch($ploopi_op)
	{
		...
		case 'tickets_delete':
			...
			$ticket = new ticket();
			if (isset($_GET['ticket_id']) && is_numeric($_GET['ticket_id']) && $ticket->open($_GET['ticket_id']) && $_SESSION['ploopi']['userid'] == $ticket->fields['id_user'])
			{
				...
				if ($ticket_dest->open($_SESSION['ploopi']['userid'], $_GET['ticket_id']))
				{

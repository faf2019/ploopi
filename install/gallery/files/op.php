        ...
        switch($ploopi_op)
        {
            case 'gallery_get_preview_photos_directory':
                ...
                if($db->numrows($resultSqlPhotosDir))
                {
                    ...
                    {
                        if($oldNameDir != $row['name']) echo '<h2 style="text-align: center; padding: 0; margin: 0;">'.ploopi_htmlentities($row['name']).'</h2>';
                        ...
                        <div class="gallery_bloc_image">
                            <div id="photo_preview_<?php echo $row['md5id']; ?>_<?php echo ploopi_htmlentities($row['version']); ?>" class="gallery_image_preview">
                            ...
                            <div  class="gallery_image_info">
                                <?php
                                    ...
                                    echo $skin->open_simplebloc(ploopi_htmlentities($objImgFile->fields['name']),'', '', $refresh);
                                    if(!empty($arrMeta))
                                    {
                                        $array_columns = array();
                                        $array_values = array();
                                        ...
                                        {
                                            $array_values[$c]['values']['meta']     = array('label' => ploopi_htmlentities($meta['meta']), 'style' => '');
                                            $array_values[$c]['values']['valeur']   = array('label' => ploopi_htmlentities($meta['value']), 'style' => '');

                ...
                foreach ($arrResources as $row)
                {
                    if ($row['rt_name'] != $strResourceType) // nouveau type de ressource => affichage séparateur
                    {
                        ...
                        <div style="border-width:1px 0;border-style:solid;border-color:#bbb;background-color:#ddd;overflow:auto;clear:both;padding:2px;">
                            <input type="checkbox" autocomplete="off" id="booking_rt<? echo $row['id_resourcetype']; ?>" style="display:block;float:left;margin:0;" onclick="ploopi_checkall(booking_resource_list_form, 'booking_resource<? echo $row['id_resourcetype']; ?>', this.checked, true);$('booking_resource_list_form').onsubmit();" />
                    ...
                    <p class="checkbox" style="background-color:<? echo ploopi_htmlentities($row['color']); ?>;" onclick="javascript:ploopi_checkbox_click(event, 'booking_resource<? echo $row['id_resourcetype'].$row['id']; ?>');">
                        <input type="checkbox" autocomplete="off" name="booking_resources[<? echo $row['id']; ?>]" id="booking_resource<? echo $row['id_resourcetype'].$row['id']; ?>" class="booking_rt<? echo $row['id_resourcetype']; ?>" value="<? echo $row['id']; ?>" <? if (!empty($arrSearchPattern['booking_resources'][$row['id']])) echo 'checked="checked"'; ?> onchange="javascript:booking_rt_autocheck(<? echo $row['id_resourcetype']; ?>); $('booking_resource_list_form').onsubmit();" />

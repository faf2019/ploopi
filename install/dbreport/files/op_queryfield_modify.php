...
if (isset($_POST['dbreport_queryfield_id']) && is_numeric($_POST['dbreport_queryfield_id']) && $objDbrQueryField->open($_POST['dbreport_queryfield_id']))
{
    ...
    $objForm->addField( new form_field('textarea', 'Fonction appliquée:<br /><em>(avant groupement)</em>', $objDbrQueryField->fields['function'], 'dbreport_queryfield_function', 'dbreport_queryfield_function') );

    $objForm->addField( new form_field('textarea', 'Fonction appliquée:<br /><em>(après groupement)</em>', $objDbrQueryField->fields['function_group'], 'dbreport_queryfield_function_group', 'dbreport_queryfield_function_group') );
    ...
    $objForm->addField( new form_hidden(0, 'dbreport_queryfield_raw_criteria') );
    $objForm->addField( new form_checkbox('<em>Utiliser la valeur brute ?<br />(sans apostrophes)</em>', 1, $objDbrQueryField->fields['raw_criteria'], 'dbreport_queryfield_raw_criteria', 'dbreport_queryfield_raw_criteria', array('style_form' => $objDbrQueryField->fields['type_criteria'] == '' ? 'display:none;' : 'display:block')) );

    $objForm->addField( new form_select('Filtre (Ou):', array('' => '(Aucun critère)') + dbreport::getCriterias(), $objDbrQueryField->fields['type_or'], 'dbreport_queryfield_type_or', 'dbreport_queryfield_type_or', array('style' => 'width:150px;', 'onchange' => "if (this.value == '') { $('dbreport_queryfield_or_form').style.display = 'none'; $('dbreport_queryfield_raw_or_form').style.display = 'none'; $('dbreport_queryfield_or').value = ''; } else { $('dbreport_queryfield_or_form').style.display = 'block'; $('dbreport_queryfield_raw_or_form').style.display = 'block'; }")) );
    ...
    $objForm->addField( new form_hidden(0, 'dbreport_queryfield_raw_or') );
    $objForm->addField( new form_checkbox('<em>Utiliser la valeur brute ?<br />(sans apostrophes)</em>', 1, $objDbrQueryField->fields['raw_or'], 'dbreport_queryfield_raw_or', 'dbreport_queryfield_raw_or', array('style_form' => $objDbrQueryField->fields['type_or'] == '' ? 'display:none;' : 'display:block')) );

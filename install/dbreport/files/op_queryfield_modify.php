...
if (isset($_POST['dbreport_queryfield_id']) && is_numeric($_POST['dbreport_queryfield_id']) && $objDbrQueryField->open($_POST['dbreport_queryfield_id']))
{
    ...
    $objForm->addField( new form_select('Fonctions "chaînes":', array('' => '(Sélectionnez une fonction à ajouter)') + array_combine($arrDbReportFunctions['string'], $arrDbReportFunctions['string']), '', null, 'dbreport_queryfield_functionlist', array('onchange' => 'dbreport_functionlist_onchange(this);')) );
    $objForm->addField( new form_select('Fonctions "maths":', array('' => '(Sélectionnez une fonction à ajouter)') + array_combine($arrDbReportFunctions['math'], $arrDbReportFunctions['math']), '', null, 'dbreport_queryfield_functionlist', array('onchange' => 'dbreport_functionlist_onchange(this);')) );
    ...
    $objForm->addField( new form_text('<em>En savoir plus:</em>', '<a target="_new" href="http://dev.mysql.com/doc/refman/5.0/fr/string-functions.html">Documentation sur les fonctions de chaînes</a><br /><a target="_new" href="http://dev.mysql.com/doc/refman/5.0/fr/mathematical-functions.html">Documentation sur les fonctions mathématiques</a>') );
    
    $objForm->addField( new form_select('Opération:', $arrDbReportOperations, $objDbrQueryField->fields['operation'], 'dbreport_queryfield_operation', 'dbreport_queryfield_operation', array('style' => 'width:200px;', 'onchange' => "if (this.value == 'intervals') { $('dbreport_queryfield_intervals_form').style.display = 'block'; } else { $('dbreport_queryfield_intervals_form').style.display = 'none'; $('dbreport_queryfield_intervals').value = ''; }")) );
    ...
    $objForm->addField( new form_select('Tri:', array('' => '(Aucun tri)') + $arrDbReportSort, $objDbrQueryField->fields['sort'], 'dbreport_queryfield_sort', 'dbreport_queryfield_sort', array('style' => 'width:150px;')) );
    ...
    $objForm->addField( new form_select('Filtre (Ou):', array('' => '(Aucun critère)') + $arrDbReportCriteria, $objDbrQueryField->fields['type_or'], 'dbreport_queryfield_type_or', 'dbreport_queryfield_type_or', array('style' => 'width:150px;', 'onchange' => "if (this.value == '') { $('dbreport_queryfield_or_form').style.display = 'none'; $('dbreport_queryfield_or').value = ''; } else { $('dbreport_queryfield_or_form').style.display = 'block'; }")) );

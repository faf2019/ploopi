...
{
	case 'bydate':
		$article_ordreby = 'timestp DESC';
	break;

	case 'bydaterev':
		$article_ordreby = 'timestp';
	break;

	case 'bypos':
	default:
		$article_ordreby = 'position';
	break;
}
...
if ($query_string != '') // recherche
{
	...
	if (file_exists('phpdig'))
	{
		...
		ob_end_clean();
		chdir('..');
		$template_body->assign_block_vars("switch_content_phpdig", array());
		$template_body->assign_vars(array(
									'PHPDIG_RESULT' => $result
									)
								);
	}
}

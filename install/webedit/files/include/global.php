...
function webedit_replace_links($objArticle, $mode, &$arrHeadings)
{
    ...
    if (!$strReplaced = $objCache->get_var())
    {
        ...
        if (preg_match_all('/(href=\"(#[^\"]*)\")/i', $strContent, $arrMatches) !== false)
        {
        ...
        if (preg_match_all('/(index\.php[^\"]+articleid=([0-9]+)[^\"]*)/i', $strContent, $arrMatches) !== false)
        {
        ...
        if (ploopi_init_module('doc', false, false, false))
        {
            ...
            if (preg_match_all('/(\.\/)?(index-quick\.php[^\"]+docfile_md5id=([a-z0-9]{32}))/i', $strContent, $arrMatches) !== false)
            {
                foreach($arrMatches[3] as $key => $md5)
                {
                    $objDocFile = new docfile();
                    if (!empty($md5) && $objDocFile->openmd5($md5)) // clé md5 présente & document trouvé
                    {
                        $arrSearch[] = $arrMatches[0][$key];
                        // ATTENTION ! _PLOOPI_BASEPATH est nécessaire pour la lecture des vidéos flash (chemin absolu sinon ne fonctionne pas)
                        $arrReplace[] = _PLOOPI_BASEPATH.'/'.ploopi_urlrewrite(html_entity_decode($arrMatches[2][$key]), doc_getrewriterules(), $objDocFile->fields['name'], null, true);
                    }
                }
            }
        }

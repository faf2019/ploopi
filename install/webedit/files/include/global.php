    ...
    if (!empty($arrHeadings['list']))
    {
        foreach($arrHeadings['list'] as $id => $fields)
        {
            ...
            switch($option)
            {
                ...
                default:
                    $link = ploopi\crypt::urlencode("admin.php?headingid={$fields['id']}");
                    $onclick = '';
                break;
            }
            ...
            foreach($articles['list'] as $id => $fields)
            {
                if (isset($treeview['list']['h'.$prefix.$fields['id_heading']]))
                {
                    ...
                    switch($option)
                    {
                        ...
                        default:
                            $link = ploopi\crypt::urlencode("admin.php?headingid={$fields['id_heading']}&op=article_modify&articleid={$fields['id']}");
                            $onclick = '';
                        break;
                    }
...
function webedit_template_assign(&$arrHeadings, &$arrShares, &$nav, $hid, $var = '', $link = '')
{
    ...
    if (isset($arrHeadings['tree'][$hid]))
    {
        ...
        foreach($arrAccessibleHeadings as $arrHeading)
        {
            ...
            $strHtmlLabel = ploopi\str::htmlentities($arrHeading['label']);
            ...
            switch($webedit_mode)
            {
                ...
                case 'display';
                    ...
                    $script = ploopi\str::urlrewrite($script = "index.php?headingid={$arrHeading['id']}", webedit_getrewriterules(), $arrHeading['label'], $arrParents);
                break;
            }
                ...
                if (empty($arrHeading['done_full']))
                {
                    ...
                    $template_body->assign_vars(
                        array(
                            ...
                            'HEADING_DESCRIPTION' => ploopi\str::htmlentities($arrHeading['description']),
                    ...
                    $template_body->assign_block_vars("switch_heading{$depth}" , array(
                        ...
                        'DESCRIPTION' => ploopi\str::htmlentities($arrHeading['description']),
            ...
            if ($arrHeading['visible'] && (!$arrHeadings['list'][$arrHeading['id']]['private'] || ($arrHeadings['list'][$arrHeading['id']]['private'] && (isset($arrShares[$arrHeadings['list'][$arrHeading['id']]['herited_private']]) || isset($_SESSION['webedit']['allowedheading'][$_SESSION['ploopi']['moduleid']][$arrHeadings['list'][$arrHeading['id']]['herited_private']]) || $arrHeadings['list'][$arrHeading['id']]['private_visible']))))
            {
                ...
                switch($webedit_mode)
                {
                    ...
                    case 'display';
                        $arrParents = array();
                        foreach(preg_split('/;/', $arrHeading['parents']) as $hid_parent) if (isset($arrHeadings['list'][$hid_parent])) $arrParents[] = $arrHeadings['list'][$hid_parent]['label'];
                        $script = ploopi\str::urlrewrite($script = "index.php?headingid={$id}", webedit_getrewriterules(), $arrHeading['label'], $arrParents);
                    break;
                }
                ...
                $template_body->assign_block_vars($localvar , array(
                    'DEPTH' => $depth,
                    'ID' => $arrHeading['id'],
                    'LABEL' => ploopi\str::htmlentities($arrHeading['label']),
                    'POSITION' => $arrHeading['position'],
                    'DESCRIPTION' => ploopi\str::htmlentities($arrHeading['description']),
                    'LINK' => $script,
                    'LINK_TARGET' => ($arrHeading['url_window']) ? 'target="_blank"' : '',
                ...
                if (isset($arrArticles[$arrHeading['id']]))
                {
                    foreach($arrArticles[$arrHeading['id']] as $row)
                    {
                        ...
                        else
                        {
                            ...
                            $scriptUrlArticle = ploopi\str::urlrewrite("index.php?headingid={$arrHeading['id']}&articleid={$row['id']}", webedit_getrewriterules(), $row['metatitle'], $arrParents);
                        }
...
{
    // Mise en cache
    $objCache = new ploopi\cache('webedit/article/'._PLOOPI_FRONTOFFICE_REWRITERULE.'/'.$objArticle->fields['id'].'/'.$objArticle->fields['lastupdate_timestp'], 86400);
        ...
        foreach($arrMatches[2] as $key => $strAnchor)
        {
            ...
            switch ($mode)
            {
                ...
                default:
                    ...
                    $arrReplace[] = str_replace($strAnchor, ploopi\str::urlrewrite("index.php?headingid={$objArticle->fields['id_heading']}&articleid={$objArticle->fields['id']}", webedit_getrewriterules(), $objArticle->fields['metatitle'], $arrParents).$strAnchor, $arrMatches[1][$key]);
                break;
            }
        }
...
{
    // Mise en cache
    $objCache = new ploopi\cache('sitemap.xml', 300);
    ...
    if (!$objCache->start())
    {
        ...
        while ($row = $db->fetchrow())
        {
            ...
            $arrLastMod = ploopi\date::gettimestampdetail($row['lastupdate_timestp']);
            ...
            $strScript = ploopi\str::xmlentities(_PLOOPI_BASEPATH.'/'.ploopi\str::urlrewrite("index.php?headingid={$row['id_heading']}&articleid={$row['id']}", webedit_getrewriterules(), $row['metatitle'], $arrParents));
        ...
        foreach($arrHeadings['list'] as $arrHeading)
        {
            ...
            $strScript = ploopi\str::xmlentities(_PLOOPI_BASEPATH.'/'.ploopi\str::urlrewrite("index.php?headingid={$arrHeading['id']}", webedit_getrewriterules(), $arrHeading['label'], $arrParents));
            $strPriority = sprintf("%.01f", $floPriority);
        ...
        while ($row = $db->fetchrow())
        {
            $strScript = ploopi\str::xmlentities(_PLOOPI_BASEPATH.'/'.ploopi\str::urlrewrite("index.php?query_tag={$row['tag']}", webedit_getrewriterules()));
        ...
        foreach($_SESSION['ploopi']['modules'] as $intIdModule => $arrModule)
        {
            ...
            {
                // init du module
                ploopi\module::init($arrModule['moduletype'], false, false, false);
...
function webedit_isEditor($heading, $user = null, $type = 'user', $id_module = null)
{
    ...
    if(empty($arrEditor) && $type == 'user')
    {
        ...
        if(!empty($arrGroups))
            $arrEditor = ploopi\validation::get(_WEBEDIT_OBJECT_HEADING_BACK_EDITOR, $arrHeading, $id_module, $arrGroups, 'group');
    }
...
function webedit_isValidator($heading, $user = null, $type = 'user', $id_module = null)
{
    ...
    if(empty($arrEditor) && $type == 'user')
    {
        ...
        if(!empty($arrGroups))
            $arrEditor = ploopi\validation::get(_WEBEDIT_OBJECT_HEADING, $arrHeading, $id_module, $arrGroups, 'group');
    }

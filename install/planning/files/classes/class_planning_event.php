...
class planning_event extends data_object
{
    ...
    public function save()
    {
        ...
        if ($this->new) 
        {
            ...
            if (!empty($this->arrDetails))
            {
                ...
                $objEventDetail->fields['timestp_begin'] = ploopi_local2timestamp($this->arrDetails['timestp_begin_d'], sprintf("%02d:%02d:00", $this->arrDetails['timestp_begin_h'], $this->arrDetails['timestp_begin_m']));
                $objEventDetail->fields['timestp_end'] = ploopi_local2timestamp($this->arrDetails['timestp_end_d'], sprintf("%02d:%02d:00", $this->arrDetails['timestp_end_h'], $this->arrDetails['timestp_end_m']));
                ...
                if (!empty($this->fields['periodicity']) && !empty($this->arrDetails['periodicity_end_date'])) // Périodicité définie
                {
                    ...
                    $intUxTsPeriodEnd = ploopi_timestamp2unixtimestamp(substr(ploopi_local2timestamp($this->arrDetails['periodicity_end_date']), 0, 8).'235959');

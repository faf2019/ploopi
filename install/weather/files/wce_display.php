...
if (file_exists("./templates/frontoffice/{$template_name}/weather.tpl"))
{
  ...
  $objWeather = new weather($obj['module_id']);
  ...
  if(isset($arrData) && is_array($arrData))
  {
    foreach($arrData as $key => $data)
    {
      ...
      {
        $template_weather->assign_block_vars('weather' , array(
        ...
        {
          $template_weather->assign_block_vars('weather.sun' , array(
            'DAWN'  => $arrData[0]['soleil_leve'],
            'DUSK'  => $arrData[0]['soleil_couche']
            )
          );
          $template_weather->assign_block_vars('weather.night' , array(
      ...
      else
      {
        ...
        {
          $template_weather->assign_block_vars('weather.this_night' , array(
        ...
        {
          $template_weather->assign_block_vars('weather.this_PM' , array(
        ...
        {
          $template_weather->assign_block_vars('weather.preview' , array(
          ...
          $template_weather->assign_block_vars('weather.preview.days' , array(
            'LARGE_PICT'  => './modules/weather/img/93/'.$data['jour']['icon'].'.png',
            'MEDIUM_PICT' => './modules/weather/img/61/'.$data['jour']['icon'].'.png',
            'SMALL_PICT'  => './modules/weather/img/31/'.$data['jour']['icon'].'.png',
          ...
          $template_weather->assign_block_vars('weather.preview.night' , array(
            'LARGE_PICT'  => './modules/weather/img/93/'.$data['nuit']['icon'].'.png',
            'MEDIUM_PICT' => './modules/weather/img/61/'.$data['nuit']['icon'].'.png',
            'SMALL_PICT'  => './modules/weather/img/31/'.$data['nuit']['icon'].'.png',
  ...
  $template_weather->pparse('weather_display');
}
?>
...
?>
<form name="form_field" action="<?php echo ovensia\ploopi\crypt::urlencode('admin.php?'.implode('&', $arrParams)); ?>" method="post" onsubmit="javascript:return forms_group_validate(this);">
<div style="overflow:hidden">
    <div style="float:left;width:50%;">
        <div class="ploopi_form" style="padding:4px;">
            <p>
                <label><?php echo _FORMS_GROUP_LABEL; ?>:</label>
                <input type="text" class="text" name="forms_group_label" value="<?php echo ovensia\ploopi\str::htmlentities($objGroup->fields['label']); ?>">
            </p>
            <p>
                <label><?php echo _FORMS_GROUP_DESCRIPTION; ?>:</label>
                <textarea class="text" name="forms_group_description"><?php echo ovensia\ploopi\str::htmlentities($objGroup->fields['description']); ?></textarea>
            </p>
            <p>
                ...
                <br />
                <input style="margin-top:2px;" type="text" class="text" name="forms_group_formula" id="forms_group_formula" value="<?php echo ovensia\ploopi\str::htmlentities($objGroup->fields['formula']); ?>">
                <?php
            ...
            <p>
                ...
                <select class="select" style="width:30%;" name="_forms_group_cond[<?php echo $intI; ?>][field]">
                    ...
                    foreach($arrFields as $intIdField => $arrField)
                    {
                        ...
                        {
                            ?>
                             <option value="<?php echo $intIdField; ?>" <?php echo (isset($arrConditions[$intI]['field']) && $intIdField == $arrConditions[$intI]['field']) ? 'selected="selected"' : ''; ?>><?php echo ovensia\ploopi\str::htmlentities($arrField['name']); ?></option>
                            <?php
                        }
                    }
                    ?>
                ...
                <select class="select" style="width:20%;" name="_forms_group_cond[<?php echo $intI; ?>][op]">
                    ...
                    {
                         ?>
                         <option value="<?php echo ovensia\ploopi\str::htmlentities($strKey, null, 'ISO-8859-1', false); ?>" <?php echo (isset($arrConditions[$intI]['op']) && $strKey == $arrConditions[$intI]['op']) ? 'selected="selected"' : ''; ?>><?php echo ovensia\ploopi\str::htmlentities($strValue, null, 'ISO-8859-1', false); ?></option>
                         <?php
                    }
                    ?>
                </select>
                <input style="width:46%;" type="text" class="text" name="_forms_group_cond[<?php echo $intI; ?>][value]" value="<?php echo isset($arrConditions[$intI]['value']) ? ovensia\ploopi\str::htmlentities($arrConditions[$intI]['value']) : ''; ?>">
            </p>
            <?php
        }
        ?>

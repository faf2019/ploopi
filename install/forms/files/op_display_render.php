...
while ($fields = $db->fetchrow($rs_fields))
{
	...
	else
	{
		...
		if ($fields['type'] == 'autoincrement')
		{
			...
			{
				$select = "SELECT max(value) as maxinc FROM ploopi_mod_forms_reply_field WHERE id_form = '{$id_form}' AND id_field = '{$fields['id']}'";

...
if ($_SESSION['ploopi']['modules'][$menu_moduleid]['doc_viewfoldersinblock'])
{
	...
	while ($row = $db->fetchrow())
	{
		...
		if ($_SESSION['ploopi']['modules'][$menu_moduleid]['doc_viewiconsinblock'])
		{
			...
			$block->addmenu("<p class=\"ploopi_va\"><img src=\"./modules/doc/img/{$ico}.png\" /><span>&nbsp;{$row['name']}</span></p>", ploopi_urlencode("{$scriptenv}?ploopi_moduleid=$menu_moduleid&ploopi_action=public&currentfolder={$row['id']}"));
		}
		else $block->addmenu($row['name'], ploopi_urlencode("{$scriptenv}?ploopi_moduleid=$menu_moduleid&ploopi_action=public&currentfolder={$row['id']}"));
...
if ($_SESSION['ploopi']['modules'][$menu_moduleid]['doc_displayroot'])
{
	...
	$block->addmenu($label, ploopi_urlencode("{$scriptenv}?ploopi_moduleid=$menu_moduleid&ploopi_action=public"));
}


if ($_SESSION['ploopi']['modules'][$menu_moduleid]['doc_displaysearch'])
{
	$block->addmenu(_DOC_SEARCH, ploopi_urlencode("{$scriptenv}?ploopi_moduleid=$menu_moduleid&ploopi_action=public&op=doc_bd_search"));
}

if (ploopi_isactionallowed(0, $_SESSION['ploopi']['workspaceid'], $menu_moduleid))
{
	$block->addmenu('<b>'._DOC_LABEL_ADMIN.'</b>', ploopi_urlencode("{$scriptenv}?ploopi_moduleid={$menu_moduleid}&ploopi_action=admin"));
}
?>

...
function wiki_internal_links($arrMatches, $intIdModule = null)
{
    ...
    if (!empty($arrMatches[1]))
    {
        ...
        return '<span class="'.$strLinkClass.'"><a title="'.$strTitle.'" href="'.ploopi\crypt::urlencode_trusted("admin.php?{$strOp}wiki_page_id=".urlencode($strPageId)).'">'.$arrMatches[1].'</a><img src="./modules/wiki/img/ico_link.png" /></span>';
    }
    ...
    if (sizeof($arrMatches) == 3)
    {
        switch($arrMatches[1][0]) // On regarde le 1er caractère du lien
        {
            ...
            default: // autres cas : liens externes
                return '<span class="wiki_link_ext"><a title="Ouvrir le lien externe &laquo; '.ploopi\str::htmlentities(strip_tags($arrMatches[1])).' &raquo;" href="'.$arrMatches[1].'">'.$arrMatches[2].'</a><img src="./modules/wiki/img/ico_link_ext.png" /></span>';
            break;
...
function wiki_highlight($strContent, $strFormat = 'php')
{
    ...
    else $strContent = '<pre>'.ploopi\str::nl2br(ploopi\str::htmlentities($strContent)).'</pre>';
    */

    $strContent = ploopi\str::htmlentities($strContent, null, null, false);
...
{
    return $arrRelevance = ploopi\search_index::search($strQueryString, _WIKI_OBJECT_PAGE, null, $rowObject['id_module']);
}

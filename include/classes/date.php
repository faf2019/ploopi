...
/**
 * Gestion des dates, des timestamps MYSQL et des fuseaux horaires.
...
abstract class date {
    ...
    public static function holiday($timestamp)
    {
        ...
        $jp = date("d", $pak);
        $mp = date("m", $pak);
        if($jp == $intDay && $mp == $intMonth) { $booIsHoliday = true;} // P̢ques

        $lpk = mktime(date("H", $pak), date("i", $pak), date("s", $pak), date("m", $pak), date("d", $pak) +1, date("Y", $pak) );
        $jp = date("d", $lpk);
        $mp = date("m", $lpk);
        if($jp == $intDay && $mp == $intMonth){ $booIsHoliday = true; } // Lundi de P̢ques
        ...
        $jp = date("d", $pe);
        $mp = date("m", $pe);
        if($jp == $intDay && $mp == $intMonth) {$booIsHoliday = true;} // Pentec̫te

        $lp = mktime(date("H", $asc), date("i", $pak), date("s", $pak), date("m", $pak), date("d", $pak) + 50, date("Y", $pak) );
        $jp = date("d", $lp);
        $mp = date("m", $lp);
        if($jp == $intDay && $mp == $intMonth) {$booIsHoliday = true;} // lundi Pentec̫te

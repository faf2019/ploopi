...
class mimethumb
{
    ...
    public function getThumbnail($pathfile)
    {
        ...
        switch(strtoupper($this->strExtension))
        {
            ...
            case 'TXT':
            case 'SQL':
                if(strtoupper($this->strExtension) == 'SQL') { $pathfile .= '.txt'; $this->strExtension = 'txt'; }
    ...
    private function _thumbImagick($arrParam = '')
    {
        ...
        try {
            ...
            {
                $thumb->setImageFormat($this->strExport);
            }
    ...
    private function _thumbJodconverter($formExport)
    {
        try {
            ...
            if(!empty($_SESSION['ploopi']['modules'][$_SESSION['ploopi']['moduleid']]['doc_jodwebservice']) && ploopi_is_url($_SESSION['ploopi']['modules'][$_SESSION['ploopi']['moduleid']]['doc_jodwebservice']))
            {
            ...
            else
            {
                ...
                // Controle du demon soffice pour jodconverter
                exec('ps -A | grep soffice',$arrResult);
                if(empty($arrResult))
                {
    ...
    private function _thumbSvg()
    {
        try {
            ...
            $strParamExport = " -f{$this->strPathFile} -e{$fileTempo} -d72";
            
            if(_PLOOPI_SERVER_OSTYPE == 'unix')
                exec('inkscape'.$strParamExport);
            elseif(file_exists('c:\\Program Files\\Inkscape.exe'))
                exec('c:\\Program Files\\Inkscape.exe'.$strParamExport);

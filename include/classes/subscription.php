...
class subscription extends data_object
{
    ...
    public static function display($id_object, $id_record, $allowedactions = null, $optional_title = '')
    {
        $ploopi_subscription_id = md5("{$_SESSION['ploopi']['moduleid']}_{$_SESSION['ploopi']['userid']}_{$id_object}_".addslashes($id_record));

        $_SESSION['subscription'][$ploopi_subscription_id] = array(
            'id_object' => $id_object,
            'id_record' => $id_record,
        ...
        <?php self::display_refresh($ploopi_subscription_id); ?>
        </div>
        <?php
    }
    ...
    public static function notify($id_object, $id_record, $id_action, $object_title, $arrUsers, $message = '')
    {

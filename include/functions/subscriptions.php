...
function ploopi_subscription_refresh($ploopi_subscription_id, $next = '')
{
    ...
    if ($booSubscribed)
    {
        $strTitle = "Vous êtes abonné {$_SESSION['subscriptions'][$ploopi_subscription_id]['optional_title']}";
    ...
    {
        $strTitle = "Vous n'êtes pas abonné {$_SESSION['subscriptions'][$ploopi_subscription_id]['optional_title']}";
        $strChecked = '';
    ...
    <div style="overflow:hidden;">
        <a id="annotations_count_<? echo $ploopi_subscription_id; ?>" class="ploopi_subscription_viewdetail" href="javascript:void(0);" onclick="javascript:ploopi_switchdisplay('<? echo $div_id; ?>');ploopi_xmlhttprequest('admin-light.php','ploopi_op=ploopi_switchdisplay&id=<? echo $div_id ?>&display='+$('<? echo $div_id ?>').style.display);">
    ...
    <div style="display:<? echo $_SESSION['ploopi']['switchdisplay'][$div_id]; ?>;" id="<? echo $div_id; ?>" class="ploopi_subscription_detail">
        ...
        <div style="float:left;width:300px;">
            ...
            if (empty($_SESSION['subscriptions'][$ploopi_subscription_id]['allowedactions'])) // pas de liste d'actions
            {
                $where = " AND id_object = {$_SESSION['subscriptions'][$ploopi_subscription_id]['id_object']} ";
            }
            else
            {
                $where = " AND id_action IN ('".implode("','", $_SESSION['subscriptions'][$ploopi_subscription_id]['allowedactions'])."')";
            }
